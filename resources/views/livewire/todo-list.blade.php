...
<div class="py-6 flex flex-col w-[300px] mx-auto">
    {{-- @if (count($todos) == 0)
        <p class="text-gray-500 text-center">There are no todos</p>
    @endif --}}

    @foreach ($todos as $todohh)
        <div class="flex gap-4 justify-between items-center py-1">
            ...
            <label class="flex-1 {{ $todohh->completed ? 'line-through' : '' }}">{{ $todohh->todo }}</label>

            <button wire:click="deleteTodo({{ $todohh->id }})">
